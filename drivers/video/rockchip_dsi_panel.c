...
struct rockchip_dsi_panel {
	...
	struct fdt_gpio_state lcd_en_gpio;
	struct fdt_gpio_state bl_en_gpio;
...
static int rockchip_dsi_panel_prepare(struct display_state *state)
{
	...
	fdtdec_set_gpio(&panel->lcd_en_gpio, 1);
	...
	fdtdec_set_gpio(&panel->bl_en_gpio, 1);
...
static int rockchip_dsi_panel_unprepare(struct display_state *state)
{
	...
	fdtdec_set_gpio(&panel->bl_en_gpio, 0);
	...
	fdtdec_set_gpio(&panel->lcd_en_gpio, 0);
...
static int rockchip_dsi_panel_parse_dt(const void *blob, int node, struct rockchip_dsi_panel *panel)
{
	...
	struct fdt_gpio_state *lcd_en_gpio = &panel->lcd_en_gpio;
	struct fdt_gpio_state *bl_en_gpio = &panel->bl_en_gpio;
	const void *data;
	int len = 0;
	int ret = 0;
	...
	fdtdec_decode_gpio(blob, node, "lcd-en-gpios", lcd_en_gpio);
	fdtdec_decode_gpio(blob, node, "bl-en-gpios", bl_en_gpio);
	...
	gpio_direction_output(lcd_en_gpio->gpio, !(lcd_en_gpio->flags & OF_GPIO_ACTIVE_LOW));
	gpio_direction_output(bl_en_gpio->gpio, !(bl_en_gpio->flags & OF_GPIO_ACTIVE_LOW));
